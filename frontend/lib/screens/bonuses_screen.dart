  ...
  Future<void> _payBonus(EmployeeBonusModel bonus) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          isAr
              ? 'هل تريد تسجيل دفع مكافأة ${bonus.amount} للموظف ${bonus.employee?.fullName ?? ""}؟'
              : 'Mark bonus of ${bonus.amount} as paid for ${bonus.employee?.fullName ?? ""}?',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
    ...
    if (confirm == true && bonus.id != null) {
      try {
        await widget.api.payBonus(bonus.id!);
        _showSnack(isAr ? 'تم تسجيل الدفع' : 'Marked as paid');
        _loadBonuses();
      } catch (e) {
        _showSnack(e.toString(), isError: true);
      }
  ...
  Future<void> _editBonus(EmployeeBonusModel bonus) async {
    ...
    if (result == true) {
      try {
        ...
        await widget.api.updateBonus(
          id: bonus.id!,
          amount: amount,
          notes: notesController.text.trim().isEmpty ? null : notesController.text.trim(),
          periodStart: dateFormat.format(start),
          periodEnd: dateFormat.format(end),
        );
