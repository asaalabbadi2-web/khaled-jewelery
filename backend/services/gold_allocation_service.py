...
def direct_linked_settlement_for_invoice(invoice_id: int) -> float:
    """Total gold settled by vouchers that explicitly name this invoice,
    main-karat-equivalent (Phase 12E mechanism A).

    Reads the GL, not the voucher's own lines: the GL is the contract's SSoT,
    and a voucher that was never posted (or was reversed) must not count as
    settlement. A supplier-tagged gold DEBIT reduces the supplier's gold
    liability -- the empirically verified direction of a real
    settlement-to-invoice voucher (Phase 12E: voucher #21 -> JE#70,
    debit_18k=90.8).
    """
    if not invoice_id:
        return 0.0
    ...
    for d18, d21, d22, d24 in rows:
        for karat, weight in ((18, d18), (21, d21), (22, d22), (24, d24)):
            ...
            if weight > 0:
                total_main_karat += float(convert_to_main_karat(weight, karat))
    return round(total_main_karat, 2)
